...
def warp_image(img, show_points=False):
    ...
    #     ]
    # )
    src_points = np.float32([[690, 450], [1110, img_size[1]], [175, img_size[1]], [595, 450]])

    offset = 300
    dest_points = np.float32(
        [[img_size[0] - offset, 0], [img_size[0] - offset, img_size[1]], [offset, img_size[1]], [offset, 0]]
    )
    # for p in src_points:
...
if __name__ == "__main__":
    ...
    for i, test_image_path in enumerate(test_images):
        test_image_path = get_input_path("output_images").joinpath("test1_undistorted.jpg")
